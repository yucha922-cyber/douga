...
- **音声入力**でプロンプト・見出し・サブテキスト・CTA をハンズフリー入力
  （Web Speech API / Chrome / Edge / Safari 対応）
- AI プロンプト一行から見出し・サブ・CTA・カラーパレットを自動生成
- **16:9 / 1:1 / 9:16 を同時編集 & 同時プレビュー**（テキスト変更が全サイズに即反映）
- カラーパレット 5 種、テンプレ 4 種、BGM 3 種をワンクリック切り替え
- 「他サイズへ自動複製」ボタンで 1 つの編集を全フォーマットに展開
- 「全サイズを MP4 で書き出し」「Premiere Pro / After Effects へ送る」
